...
~~~
  ...
  The [**`.c_strings()`**](../../source/library/process/Commandline.hpp#L169) member function of the Kickstart command line singleton, produces an `Array_span_<const C_str>`, which is essentially a pair of pointers into an array of `C_str` . I.e. Kickstart’s [**`Array_span_`**](../../source/library/core/collection-util.hpp#L68) corresponds to a pared down C++20 [`std::span`](https://en.cppreference.com/w/cpp/container/span) with dynamic extent. The [**`C_str`**](../../source/library/core/language/type-aliases.hpp#L37) type is technically even simpler, it’s just an alias for `const char*`, but the name implies a zero-terminated string.
