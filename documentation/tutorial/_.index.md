...
The Kickstart way to access command line arguments is to add two parameters to your `cppmain` (or whatever you call that function), as shown below:
...
Result in a classic Windows console, with a `cpchp` command reporting the active codepage at the start:
...
Directly standard library based code, on the other hand, is generally Not A Good Idea&trade; for handling general file names as command line argument arguments, in Windows, or in portable code. Such code uses parameters of `main`. And as of early 2021 all Windows C++ implementations encode those parameters with Windows ANSI, which maps to an encoding that depends on the system, but which rarely is UTF-8.
