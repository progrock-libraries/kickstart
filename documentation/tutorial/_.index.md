...
In Unix environments, which includes the Mac, as of 2021 the arguments can in practice be relied on to be UTF-8 encoded.

You can therefore pass these “raw” command line arguments to the Kickstart `with_arguments_displayed` function, which by default will do one of two things:
...
* on other systems the passed command line arguments are used to initialize the command line singleton, after a check that they’re valid UTF-8.
