...
Here **`Array_span_`** is a basic array span class template. Kickstart’s `Array_span_` corresponds to a pared down C++20 [`std::span`](https://en.cppreference.com/w/cpp/container/span) with dynamic extent; just a pair of pointers into an array. The **`C_str`** type is technically even simpler, just an alias for `const char*`, but the name implies a zero-terminated string.

**`int_size`** is a general convenience function for collection classes, which avoids a `static_cast<int>` of the size, and **`begin_ptr_of`** is likewise just a convenience function, that returns a pointer to the first item.
