...
Now the round apostrophe is correctly presented. Codepage 65001 works for UTF-8 *output*. But the non-ASCII characters in the name “Bjørn Håvard Sæther” are not handled correctly: they end up being presented as spaces! Actually the Windows API level’s `ReadFile` function produces a string with zero values for these input characters, so codepage 65001 decidedly does not work for *input*.

And as of early 2021 this happens with C++ standard library input whether one uses a classic console or Microsoft Terminal.

---

When Kickstart’s `input()` detects End Of File&trade; it throws an exception, while `std::cin` by default just fails silently, which can wreak all kinds of serious havoc (a crash, a hang, or the insidiously plausible but incorrect result that incurs devastating costs for a company).

The exception is much more safe.

In Ubuntu the exception, including the type and exception message text, is reported by the C++ run time support. However, unfortunately, with the Visual C++ and MinGW g++ compilers in Windows 10 the exception just causes a short pause and then a silent program termination. To ensure that such exceptions are reported you can replace your `main` function with a function named e.g. `cppmain`, and provide a one-liner `main` that calls that via Kickstart’s **`with_exceptions_displayed`** function:
