...
The main Kickstart way to access command line arguments is to access it via the **`process::the_commandline()`** function, which returns a reference to a static object:
...
`process` is a namespace for process related functionality. I.e. it doesn’t indicate that `process::the_commandline()` does things with the command line, such as parsing it. Although it actually does that in Windows; hm.
...
* *as of early 2021 this functionality is only implemented for Windows and Linux.*

In particular there’s as yet no implementation for the Mac. So what to do?

---

Directly standard library based code, using parameters of `main`, can work cross platform for accessing pure ASCII command line arguments:



However that’s Not A Good Idea&trade; for handling general file names as command line argument arguments, in Windows, or in portable code.

As of early 2021 all Windows C++ implementations encode those parameters with Windows ANSI, which maps to an encoding that depends on the system, but which rarely is UTF-8.

And when it’s not UTF-8 then it usually can’t represent the
