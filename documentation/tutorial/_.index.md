...
Note: this example’s “π.txt” file is encoded with UTF-8 with a **BOM** (a *byte order mark*) at the start, which is the convention in Windows. The BOM character is suppressed by Kickstart’s console output operations. In fact all BOM characters — historically they were “zero width no-break space” characters that shouldn’t be rendered — are suppressed for console output, but they’re retained when the output goes to a file such as with i/o redirection.

In the above code:

* The `Text_reader` constructor accepts an UTF-8 encoded path string, or a `std::filesystem::path` (the formal argument type is a Kickstart [**`Path`**](../../source/library/core/stdlib-extensions/filesystem/Path.hpp)).
...
As of early 2021 the Windows implementations of the C++ standard library simply do not support opening a C `FILE*` from UTF-8 path, BOM suppression for console output, or for that matter guaranteed correct output of non-ASCII text such as the “…” above, so this is an example where a library like Kickstart is absolutely *necessary* in order to express a reasonable beginner’s simple task in C++.
