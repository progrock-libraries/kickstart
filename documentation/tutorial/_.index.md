...
- [**3. Text i/o.**](#3-text-io)
  - [**3.1. Output of text with non-English letters like Norwegian ÆØÅ.**](#31-output-of-text-with-non-english-letters-like-norwegian-%C3%A6%C3%B8%C3%A5)
  - [**3.2. Input of text with non-English letters like Norwegian ÆØÅ.**](#32-input-of-text-with-non-english-letters-like-norwegian-%C3%A6%C3%B8%C3%A5)
...
- [**4. Command line arguments.**](#4-command-line-arguments)
  ...
  - [**4.3 Command line options parsing via a 3ʳᵈ party library.**](#43-command-line-options-parsing-via-a-3%CA%B3%E1%B5%88-party-library)
...
Instead, where you want to e.g. place some ASCII output right-adjusted in a field *n* characters wide, you just use the [**`ascii::at_right_in`**](../../source/library/core/text-encoding-ascii/string-util.hpp#L81) function. The “ASCII” is not really a restriction to ASCII characters, but just that each `char` value in the string should produce one character cell in the console. However, with UTF-8 encoding all non-ASCII characters use two or more `char` values. There’s also a corresponding `at_left_in`, of course. These functions take the field width *n* as an `int` 1ˢᵗ parameter, and the something to be displayed as a `std::string_view` 2ⁿᵈ parameter.
...
### **4.3 Command line options parsing via a 3ʳᵈ party library.**
...
*File ([program-options.direct-cxxopts.cpp](examples/program-options.direct-cxxopts.cpp)):*
~~~cpp
...
The [**`.c_strings()`**](../../source/library/process/Commandline.hpp#L169) member function of the Kickstart command line singleton produces an `Array_span_<const C_str>`. An [**`Array_span_`**](../../source/library/core/collection-util.hpp#L68)  is essentially a pair of pointers into an array, representing a contiguous part of that array, a “[span](https://en.cppreference.com/w/cpp/container/span)”. The array item type [**`C_str`**](../../source/library/core/language/type-aliases.hpp#L37) is just an alias for `const char*`, but the name implies a zero-terminated string.

 [**`int_size`**](../../source/library/core/collection-util.hpp#L43) is a general convenience function for collection classes, which avoids a `static_cast<int>` of the size, and [**`begin_ptr_of`**](../../source/library/core/collection-util.hpp#L48) is likewise just a convenience function, that returns a pointer to the first item.

Subtlety: the global reference `cmd`, used above to access the command line singleton, is obtained before the singleton has been initialized with the arguments of `main`. That works *by design*, as convenience functionality. You can obtain a reference to the singleton at any time, but you cannot initialize it (e.g. with the arguments of `main`) after data has been retrieved from it.

And it can only be explicitly initialized once: it's logically `const`.
