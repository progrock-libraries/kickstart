...
When `input()` detects End Of File&trade; it throws an exception, while `std::cin` by default just fails silently, which can wreak all kinds of serious havoc.

In Ubuntu the exception, including the type and exception message text, is reported by the C++ run time support. However, with the Visual C++ and MinGW g++ compilers in Windows 10 the exception just causes a silent program termination. In order to ensure that such exceptions are reported you can replace your `main` with a function named e.g. `cppmain`, and provide a one-liner `main` that calls that via Kickstart’s **`with_exceptions_displayed`** function:
