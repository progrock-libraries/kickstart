...
So far we’ve seen that Kikstart `input()` works correctly for common normal input such as a Norwegian name, while, with current C++ standard library implementations, `std::cin` does not.

In addition, when `input()` detects End Of File&trade; it throws an exception, while `std::cin` just fails silently, which means that with the standard library input the program can go on to produce erroneous but plausible results.
