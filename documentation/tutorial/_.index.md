...
When Kickstart’s `input()` detects End Of File&trade; it throws an exception, while `std::cin` by default just fails silently, which can wreak all kinds of serious havoc: a crash, a hang, or the insidious plausible but incorrect result that is believed and incurs devastating costs for a company&hellip;

The exception is much more safe.

In Ubuntu an exception out of `main` is reported by the C++ run time support, including the type and exception message text. However, unfortunately, with the Visual C++ and MinGW g++ compilers in Windows 10 an exception out of `main` just causes a short pause and then a silent program termination. To ensure that such exceptions are reported you can replace your `main` function with a function named whatever you like but e.g. `cppmain`, and provide a one-liner little micro-`main` that calls that function via Kickstart’s **`with_exceptions_displayed`**:
