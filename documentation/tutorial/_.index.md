...
When Kickstart’s `input()` detects End Of File&trade; it throws an exception, while `std::cin` by default just fails silently, which can wreak all kinds of serious havoc.
