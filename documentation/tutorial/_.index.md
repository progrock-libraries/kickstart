...
*File ([io/text-files/display.cpp](examples/io/text-files/display.cpp)):*
~~~cpp
#include <kickstart/all.hpp>
using namespace kickstart::all;
...
Files that are closely tied to a necessary for a program are most naturally placed in the same directory as the executable, or a sub-directory there.

 Unfortunately with the standard C++ library there is no reliable way to find the executable’s directory. About the closest one get get is to search the `PATH` directorys as well as other relevant directories, for a match for the command indicated by `main` parameter `argv[0]`. However, Windows adds a number of “other relevant directories”, and the value of `argv[0]` is not guaranteed.
...
In addition to **`path_of_exe_relative()`** there is **`path_of_exe_directory()`** and **`path_of_executable()`**, plus a ditto set of functions for `std::filesystem::path` results in case one needs that representation.
...
The Windows API call accesses information that’s already in memory, so it’s presumably quite efficient. The Linux code has to open and close a file, so it’s presumably noticeably slower. And the general Unix approach fires up two processes in a pipeline, so it’s presumably very slow. For this reason when the executable’s directory path has been determined the first time, it’s cached. Subsequent use of that directory path should be super-fast.
